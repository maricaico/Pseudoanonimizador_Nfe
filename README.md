...
- NCM, CFOP e CEST, quando presentes
- Município, UF e bairro
- Os 5 primeiros dígitos do CEP, preservando uma referência geográfica agregada
- Datas
- Série do documento

A preservação desses campos busca manter a utilidade analítica dos XMLs.
...
Campos preservados por necessidade analítica, como bairro, prefixo do CEP e descrição de produtos (`xProd`), devem ser considerados na avaliação de risco do conjunto de dados, pois podem manter informações indiretas sobre localização, produtos ou contexto comercial.